...
const LaundryIcon = ({ className = 'size-[18px] shrink-0' }: { className?: string }) => (
...
const TX_TYPE_ICON: Record<string, React.ReactNode> = {
  ...
  'Dryer':  <LaundryIcon />,
  'Funds Added': (
    <svg className="size-[18px] shrink-0" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const Checkbox = ({ checked, onChange, disabled = false, indeterminate = false }: {
  checked: boolean;
  onChange?: () => void;
  disabled?: boolean;
  ...
  >
    {indeterminate && !checked ? (
      ...
      <svg className="size-[10px] text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3.5}>
    ...
    <div className="flex gap-5 px-6 py-5 overflow-y-auto flex-1">
      ...
      <div className="flex flex-col gap-4 flex-1 min-w-0">
        <div className="border border-border rounded-lg p-4">
          ...
          <div className="flex items-center justify-between py-2.5 rounded gap-4 bg-ds-orange-100 px-3 -mx-3">
            <p className="text-ds-body-sm text-ds-yellow-900">Last 6 months</p>
            <p className="text-ds-label font-semibold text-ds-yellow-900">1</p>
          </div>
        </div>
...
const BulkRefundModal = ({ transactions, onClose, onSuccess }: {
  transactions: Transaction[];
  onClose: () => void;
  onSuccess: (txs: Transaction[], reason: string, note: string) => void;
}) => {
  const [reason, setReason] = useState('');
  const [note, setNote] = useState('');
  const [showReasons, setShowReasons] = useState(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      <div className="absolute inset-0 bg-black/50" onClick={onClose} />
      <div className="relative bg-white rounded-lg border border-border shadow-xl flex flex-col w-full max-w-[540px] max-h-[90vh]">
        <div className="flex items-start justify-between px-6 pt-6 pb-4 shrink-0">
          <div>
            <p className="text-ds-h4 font-semibold text-text-body">Create Refund Request</p>
            <p className="text-ds-body-sm text-text-muted mt-0.5">{transactions.length} transaction{transactions.length !== 1 ? 's' : ''} selected</p>
          </div>
          <button onClick={onClose} className="text-text-muted hover:text-text-body transition-colors mt-1">
            <svg className="size-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-5">
          ...
          <div className="border border-border rounded-lg overflow-hidden">
            <div className="flex bg-surface px-3 py-2">
              <p className="text-ds-caption font-medium text-text-muted flex-1">Order / Details</p>
              <p className="text-ds-caption font-medium text-text-muted w-[80px] text-right">Amount</p>
            </div>
            {transactions.map((tx, i) => (
              <div key={tx.id} className={cn('flex items-center px-3 py-2.5', i > 0 ? 'border-t border-border' : '')}>
                <div className="flex-1 min-w-0">
                  <p className="text-ds-body-sm font-medium text-text-body">Order #{tx.orderId}</p>
                  <p className="text-ds-caption text-text-muted truncate">{tx.details.split('—')[0].trim()} · {tx.date}</p>
                </div>
                <p className="text-ds-body-sm font-semibold text-text-body w-[80px] text-right shrink-0">-${tx.amount.toFixed(2)}</p>
              </div>
            ))}
            <div className="flex items-center px-3 py-2.5 border-t border-border bg-surface">
              <p className="text-ds-label font-semibold text-text-body flex-1">Total</p>
              <p className="text-ds-label font-semibold text-text-body w-[80px] text-right shrink-0">-${total.toFixed(2)}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              {showReasons && (
                <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-border rounded-lg shadow-md z-10 overflow-hidden">
                  {REFUND_REASONS.map((r) => (
                    <button key={r} className="w-full px-4 py-3 text-left text-ds-body-sm hover:bg-primary-50 transition-colors"
                      onClick={() => { setReason(r); setShowReasons(false); }}>
                      {r}
                    </button>
                  ))}
          ...
          <div className="flex flex-col gap-2">
            <p className="text-ds-body-sm text-text-muted">Note</p>
            <textarea
              className="w-full h-[100px] border border-border rounded-lg px-4 py-3 text-ds-body-sm text-text-body placeholder:text-text-muted outline-none focus:border-primary resize-none transition-colors"
        ...
        <div className="border-t border-border shrink-0 px-6 py-4">
          <button
            disabled={!reason}
            onClick={() => onSuccess(transactions, reason, note)}
            className="w-full h-[52px] bg-black text-white text-ds-button font-semibold rounded-lg hover:opacity-90 disabled:opacity-40 transition-opacity"
          >
            Submit {transactions.length} Refund Request{transactions.length !== 1 ? 's' : ''}
          </button>
        </div>
      </div>
    </div>
...
const CustomerProfilePage = () => {
  ...
  const isRefundEligible = (tx: Transaction) =>
    (tx.type === 'Washer' || tx.type === 'Dryer') && tx.status === 'Completed' && !tx.refundStatus;

  const handleBulkRefundCreated = (txs: Transaction[], reason: string, note: string) => {
    const now = new Date();
    const refundDate = now.toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' });
    const refundTime = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    const agentFirst = userData?.loggedInUserData?.firstName ?? '';
    ...
    setSuccessAlert(`Refund requests for ${txs.length} order${txs.length !== 1 ? 's' : ''} submitted successfully.`);
  };
  ...
  return (
    <div className="flex flex-col px-8 py-6 min-h-full max-w-[1400px] mx-auto w-full">
      ...
      {showBulkRefundModal && (
        <BulkRefundModal
          transactions={allTransactions.filter((tx) => selectedTxIds.has(tx.id))}
          onClose={() => setShowBulkRefundModal(false)}
          onSuccess={handleBulkRefundCreated}
        />
      )}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="border border-border rounded-lg shadow-[0px_2px_8px_rgba(0,0,0,0.08)] bg-white p-6">
          ...
          {selectedTxIds.size > 0 && (
            <div className="flex items-center gap-3 px-4 py-2.5 bg-primary-50 border border-primary-100 rounded-lg">
              <p className="text-ds-body-sm text-primary font-medium flex-1">
                {selectedTxIds.size} transaction{selectedTxIds.size !== 1 ? 's' : ''} selected
              </p>
              <button
                onClick={() => setSelectedTxIds(new Set())}
                className="text-ds-body-sm text-text-muted hover:text-text-body transition-colors"
              >
                Clear
              </button>
              <button
                onClick={() => setShowBulkRefundModal(true)}
                className="flex items-center gap-2 h-[34px] px-4 bg-black text-white rounded-lg text-ds-body-sm font-medium hover:opacity-90 transition-opacity"
              >
                Create Refund ({selectedTxIds.size})
              </button>
            </div>
          )}
          ...
          <div className="border border-[#efefef] rounded-lg overflow-hidden">
            ...
            <div className="flex bg-[#f9fafb]">
              ...
              <div className="w-[44px] shrink-0 p-[10px] flex items-center justify-center">
                {(() => {
                  const eligibleOnPage = paginated.filter(isRefundEligible);
                  const allChecked = eligibleOnPage.length > 0 && eligibleOnPage.every((tx) => selectedTxIds.has(tx.id));
                  const someChecked = eligibleOnPage.some((tx) => selectedTxIds.has(tx.id));
                  return (
                    <Checkbox
                      checked={allChecked}
                      ...
                      onChange={() => {
                        if (allChecked) {
                          setSelectedTxIds((prev) => { const next = new Set(prev); eligibleOnPage.forEach((tx) => next.delete(tx.id)); return next; });
                        } else {
                          setSelectedTxIds((prev) => { const next = new Set(prev); eligibleOnPage.forEach((tx) => next.add(tx.id)); return next; });
                        }
                      }}
                    />
                  );
            ...
            {paginated.length === 0 ? (
              ...
              paginated.map((tx, i) => (
                ...
                >
                  {/* Checkbox */}
                  <div
                    className="w-[44px] shrink-0 px-[10px] py-[8px] flex items-center justify-center"
                  ...
                  <div className="w-[180px] shrink-0 px-[10px] py-[8px] flex items-center">
                    <span className={cn('inline-flex items-center gap-1 text-ds-caption font-medium px-2 h-[24px] rounded-[4px] whitespace-nowrap', TX_TYPE_STYLE[tx.type] ?? 'bg-surface text-text-subtle')}>
                      {TX_TYPE_ICON[tx.type] ?? null}
                      {TX_TYPE_LABEL[tx.type] ?? tx.type}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0 px-[10px] py-[8px] flex items-center">
                    <div className="min-w-0 w-full">
                      ...
                      {tx.refundStatus && (
                        <p className={cn('text-ds-caption truncate', {
                          'text-ds-yellow-900': tx.refundStatus === 'Refund Requested' || tx.refundStatus === 'Refund Pending',
                          'text-ds-green-900': tx.refundStatus === 'Refund Complete',
                          'text-ds-red-700': tx.refundStatus === 'Refund Denied',
                        })}>
                          {tx.refundStatus === 'Refund Complete'
                  ...
                  <div className="w-[90px] shrink-0 px-[10px] py-[8px] flex items-center">
                  ...
                  <div className="w-[100px] shrink-0 px-[10px] py-[8px] flex items-center">
                    <p className="text-ds-body-sm text-text-body">
                      {`$${Math.max(0, runningBalanceMap[tx.id] ?? 0).toFixed(2)}`}
                    </p>
                  </div>
                  <div className="w-[160px] shrink-0 px-[10px] py-[8px] flex items-center">
                    {tx.refundStatus ? (
                      <span className={cn('inline-flex items-center text-ds-caption font-medium px-2 h-[24px] rounded-[4px] border whitespace-nowrap', REFUND_STATUS_STYLE[tx.refundStatus])}>
                        {tx.refundStatus}
                      </span>
                    ) : (
                      <span className={cn('inline-flex items-center text-ds-caption font-medium px-2 h-[24px] rounded-[4px] border whitespace-nowrap', TX_STATUS_STYLE[tx.status])}>
                        {tx.status}
                      </span>
                    )}
                  </div>
